...
import com.streamgrabber.structures.YaAlbum;
...
public class YaStreamGrabber extends StreamGrabber {
	...
	@Override
	public synchronized void requestTrackList(final String trackTitle,
			final int maxTracksCount) throws IOException {// TODO:add
															// multithreading
		...
		while (tracksLeft > 0) {
			...
			String responce = httpProxy.makeRequestForString(HTTPProxy
					.composeRequest(
							YaHTTPProxy.LIST_OF_CONTENT_REQUEST,
							YaHTTPProxy.LIST_OF_CONTENT_KEY_PARAMS,
							new String[] { trackTitle, "tracks",
	...
	public InputStream openDownloadStream(String trackId) {
		...
		try {
			...
			String trackNameXML = httpProxy.makeRequestForString(String.format(
					"http://storage.music.yandex.ru/get/%s/2.xml",
					trackInfo.getStorageDir()));

			String fileName = HTTPProxy.getAttributeValue(trackNameXML,
					"filename");
			...
			String downloadInfoXML = httpProxy
					.makeRequestForString(String
			...
			downloadStruct.host = HTTPProxy.getElementInnerText(
					downloadInfoXML, "host");
			downloadStruct.path = HTTPProxy.getElementInnerText(
					downloadInfoXML, "path");
			downloadStruct.region = HTTPProxy.getElementInnerText(
					downloadInfoXML, "region");
			downloadStruct.ts = HTTPProxy.getElementInnerText(downloadInfoXML,
					"ts");
			downloadStruct.s = HTTPProxy.getElementInnerText(downloadInfoXML,
					"s");
			...
			String downloadRequest = String
					.format("http://%s/get-mp3/%s/%s%s?track-id=%s&/region=%s&from=service-search",
							downloadStruct.host, secretKey, downloadStruct.ts,
							downloadStruct.path, trackInfo.getTrackId(),
	...
	@Override
	public void requestTrackList(final String albumId) throws IOException {

		try {
			String htmlPageWithAlbum = new YaHTTPProxy()
					.makeRequestForString("http://music.yandex.ru/fragment/album/"
							+ albumId);
			YaAlbum yaAlbum = new YaAlbum();
		
			yaAlbum.fillInfo(htmlPageWithAlbum);
			trackList = yaAlbum.getTrackList();
